...
class NewEvent extends React.Component {
    ...
    handleSubmit(event) {
        ...
        if(res) {  
            ...
            axios
            .put(baseURL, info,  {headers:{'Authorization': `Token ${user}`}}).then((response) => {
                if(response.status === 200 || response.status === 201) {
                    alert('The event is modified.')
    ...
    render() {
        ...
        const event_id = parseInt(window.location.pathname.split('/')[2])
                ...
                <form onSubmit={this.handleSubmit}>
                    <Grid>
                        <Paper elevation={10} style={paperStyle}>
                            ...
                            <Button type='submit' color='primary' variant="contained" style={btnstyle} fullWidth>Modify Event</Button>
                            <Typography >
